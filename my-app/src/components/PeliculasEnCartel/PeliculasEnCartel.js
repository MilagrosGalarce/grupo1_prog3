...
class PeliculasEnCartel extends Component {
  ...
  componentDidMount() {
    fetch(
      'https://api.themoviedb.org/3/movie/now_playing?api_key=f9fed29318027d1571e2d4e385ce272d&language=es-ES&page=1'
    )
        ...
        {this.state.peliculas.length === 0 ? (
          ...
          <section className="row cards" id="now-playing">
            {this.state.peliculas.map((peli, idx) =>
              idx < 4 ? (
                <article className="single-card-playing" key={peli.id}>
                  <img
                    src={
                      peli.poster_path
