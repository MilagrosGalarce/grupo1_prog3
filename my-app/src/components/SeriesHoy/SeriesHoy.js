...
class SeriesHoy extends Component {
  ...
  componentDidMount() {
    fetch(
      'https://api.themoviedb.org/3/tv/airing_today?api_key=f9fed29318027d1571e2d4e385ce272d&language=es-ES&page=1'
    )
        ...
        {this.state.series.length === 0 ? (
          ...
          <section className="row cards" id="on-air-today">
            {this.state.series.map((tv, idx) =>
              idx < 4 ? (
                <article className="single-card-on-air mb-3" key={tv.id}>
                  <img
                    src={
                      tv.poster_path
